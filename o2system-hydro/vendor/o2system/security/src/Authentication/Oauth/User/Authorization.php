...
class Authorization
{
    ...
    protected function createPayload(Oauth\Client\Account $client, array $scope = [])
    {
        ...
        $payload = [
            'id'         => $id, // the internal id of the token
            'jti'        => $id, // a unique token identifier for the token (JWT ID)
            'iss'        => $this->config[ 'issuer' ], // the id of the server who issued the token (Issuer)
            'aud'        => $client->id, // the id of the client who requested the token (Audience)
            'sub'        => ($this->user->offsetExists('id') ? $this->user->offsetGet('id') : null), // the id of the user for which the token was released (Subject)
            'exp'        => $expires,
            'iat'        => time(),
            'token_type' => $this->config[ 'token' ][ 'type' ],
